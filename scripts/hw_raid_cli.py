...
class MegaRAID():
            ...
            if cmd_exit == 0:
                ...
                for line in iter(stdout.splitlines()):
                    ...
                    if dash_line_cnt == 2:
                        ...
                        print 'MegaRAID,%s,%s,%s,%s,%s,%s' % (each, encl_id, slot_num, state, size, model)
                ...
                for line in iter(stdout.splitlines()):
                    ...
                    if dash_line_cnt == 2:
                        ...
                        if len(l_drive_info) == 10:
                            name = l_drive_info[9]
                        print 'MegaRAID,%s,%s,%s,%s,%s,%s' % (each, ld_id, raid_lvl, state, size, name)
    ...
    def rem_log_drive(self, ctrlr_id, ld_id):
        # All arguments are required, so build the command and execute it
        command = STORCLI_BIN + ' /c' + str(ctrlr_id) + '/v' + str(ld_id) + ' del force 2>&1'
        process = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
        stdout, stderr = process.communicate()
        cmd_exit = process.returncode
        if cmd_exit != 0:
    ...
    def add_hsp_drive(self, ctrlr_id, encl_id, slot_num):
        # All arguments are required, so build the command and execute it
        command = STORCLI_BIN + ' /c' + str(ctrlr_id) + '/e' + str(encl_id) + '/s' + \
                  str(slot_num) + ' add hotsparedrive 2>&1'
        process = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
        stdout, stderr = process.communicate()
        cmd_exit = process.returncode
        if cmd_exit != 0:
    ...
    def rem_hsp_drive(self, ctrlr_id, encl_id, slot_num):
        # All arguments are required, so build the command and execute it
        command = STORCLI_BIN + ' /c' + str(ctrlr_id) + '/e' + str(encl_id) + '/s' + \
                  str(slot_num) + ' delete hotsparedrive 2>&1'
        process = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
        stdout, stderr = process.communicate()
        cmd_exit = process.returncode
        if cmd_exit != 0:
...
class AACRAID():
                ...
                for line in iter(stdout.splitlines()):
                    ...
                    if found_drive and 'Total Size' in line:
                        ...
                        if avail_only:
                            if pd_in_use:
                                continue
                        print 'AACRAID,%s,%s,%s,%s,%s,%s' % (each, encl_id, slot_num, state, size, model)
            ...
            if cmd_exit == 0:
                ...
                for line in iter(stdout.splitlines()):
                    ...
                    if found_drive and '   Size' in line:
                        ...
                        # This is the last line to find, so we're done here
                        found_drive = False
                        print 'AACRAID,%s,%s,%s,%s,%s,%s' % (each, ld_id, raid_lvl, state, size, name)

    def add_log_drive(self, ctrlr_id, raid_lvl, phys_drives):
        # All arguments are required, so build the command and execute it
        command = ARCCONF_BIN + ' create ' + str(ctrlr_id) + ' logicaldrive max ' + raid_lvl
    ...
    def rem_log_drive(self, ctrlr_id, ld_id):
        # All arguments are required, so build the command and execute it
        command = ARCCONF_BIN + ' delete ' + str(ctrlr_id) + ' logicaldrive ' + str(ld_id) + ' noprompt 2>&1'
        process = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
        stdout, stderr = process.communicate()
        cmd_exit = process.returncode
        if cmd_exit != 0:
    ...
    def add_hsp_drive(self, ctrlr_id, encl_id, slot_num):
        # All arguments are required, so build the command and execute it
        command = ARCCONF_BIN + ' setstate ' + str(ctrlr_id) + ' device ' + str(encl_id) + ' ' + \
                  str(slot_num) + ' hsp noprompt 2>&1'
        process = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
        stdout, stderr = process.communicate()
        cmd_exit = process.returncode
        if cmd_exit != 0:
...
def main():
    ...
    command_group = OptionGroup(parser, 'Commands', 'Use only one command option at a time; '
                                                    'combine with other options below.')
    command_group.add_option('--list-controllers', dest='list_controllers', action='store_true',
                             help='list available RAID controllers')
    command_group.add_option('--list-physical-drives', dest='list_physical_drives', action='store_true',
                             help='list all physical drives')
    command_group.add_option('--list-logical-drives', dest='list_logical_drives', action='store_true',
                             help='list all logical drives')
    command_group.add_option('--add-logical-drive', dest='add_logical_drive', action='store_true',
                             help='add a new logical drive')
    command_group.add_option('--rem-logical-drive', dest='rem_logical_drive', action='store_true',
                             help='remove an existing logical drive')
    command_group.add_option('--add-hot-spare', dest='add_hot_spare', action='store_true',
                             help='add a new global hot spare drive')
    command_group.add_option('--rem-hot-spare', dest='rem_hot_spare', action='store_true',
    ...
    common_group.add_option('--type', dest='type', help='the RAID controller hardware type',
                            metavar='TYPE', type='choice', choices=CTRLR_TYPES)
    common_group.add_option('--ctrlr-id', dest='ctrlr_id', help='the RAID controller ID',
                            metavar='ID_NUM', type='int')
    parser.add_option_group(common_group)

    pdrives_group = OptionGroup(parser, 'Specific Options for the "--list-physical-drives" Command')
    pdrives_group.add_option('--avail-only', dest='avail_only', action='store_true',
                             help='only display available/unused physical drives')
    parser.add_option_group(pdrives_group)

    add_ld_group = OptionGroup(parser, 'Specific Options for the "--add-logical-drive" Command')
    add_ld_group.add_option('--raid-level', dest='raid_level', help='the RAID level of the new logical drive',
                            metavar='LEVEL', type='string')
    add_ld_group.add_option('--phys-drives', dest='phys_drives',
                            help='the enclosure/slot string of the physical drives (eg, "0:1,0:2") '
                                 'for the new logical drive',
                            metavar='DRIVES', type='string')
    add_ld_group.add_option('--no-read-cache', dest='no_read_cache', action='store_true',
                            help='don\'t enable read-ahead cache (default is enabled)')
    add_ld_group.add_option('--no-write-cache', dest='no_write_cache', action='store_true',
                            help='don\'t enable write-back cache (default is enabled)')
    parser.add_option_group(add_ld_group)

    rem_ld_group = OptionGroup(parser, 'Specific Options for the "--rem-logical-drive" Command')
    rem_ld_group.add_option('--rem-ld-id', dest='rem_ld_id', help='the logical drive ID number of the LD to remove',
                            metavar='ID_NUM', type='int')
    parser.add_option_group(rem_ld_group)

    add_hsp_group = OptionGroup(parser, 'Specific Options for the "--add-hot-spare" Command')
    add_hsp_group.add_option('--add-encl-id', dest='add_encl_id', help='the enclosure ID of the physical drive to add',
                             metavar='ID_NUM', type='int')
    add_hsp_group.add_option('--add-slot-num', dest='add_slot_num', help='the slot number of the physical drive to add',
                             metavar='ID_NUM', type='int')
    parser.add_option_group(add_hsp_group)

    rem_hsp_group = OptionGroup(parser, 'Specific Options for the "--rem-hot-spare" Command')
    rem_hsp_group.add_option('--rem-encl-id', dest='rem_encl_id', help='the enclosure ID of the hot spare to remove',
                             metavar='ID_NUM', type='int')
    rem_hsp_group.add_option('--rem-slot-num', dest='rem_slot_num', help='the slot number of the hot spare to remove',
                             metavar='ID_NUM', type='int')
    ...
    if cmd_count > 1:
        parser.error('all command options are mutually exclusive, pick only one')

    # Create objects for the RAID CLI tools
    megaraid = MegaRAID()
    aacraid = AACRAID()

    # Perform checks for the specific command option, and execute the action if it's all good
    if options.list_controllers:
        ...
        if options.type is not None or options.ctrlr_id is not None:
            if not (options.type is not None and options.ctrlr_id is not None):
                ...
                if options.type == 'MegaRAID':
                    if megaraid.working:
                        megaraid.list_phy_drives(options.ctrlr_id, options.avail_only)
                elif options.type == 'AACRAID':
                    if aacraid.working:
                        aacraid.list_phy_drives(options.ctrlr_id, options.avail_only)
        else:
            if megaraid.working:
        ...
        if not (options.raid_level is not None and options.phys_drives is not None):
            parser.error('--raid-level and --phys-drives are required options')
        if options.type == 'MegaRAID':
            if megaraid.working:
                megaraid.add_log_drive(options.ctrlr_id, options.raid_level, options.phys_drives)
        elif options.type == 'AACRAID':
            if aacraid.working:
                aacraid.add_log_drive(options.ctrlr_id, options.raid_level, options.phys_drives)
        ...
        if not (options.type is not None and options.ctrlr_id is not None):
            parser.error('--type and --ctrlr-id are required options')
        if not (options.add_encl_id is not None and options.add_slot_num is not None):
            parser.error('--add-encl-id and --add-slot-num are required options')
        if options.type == 'MegaRAID':
            if megaraid.working:
                megaraid.add_hsp_drive(options.ctrlr_id, options.add_encl_id, options.add_slot_num)
        elif options.type == 'AACRAID':
            if aacraid.working:
                aacraid.add_hsp_drive(options.ctrlr_id, options.add_encl_id, options.add_slot_num)

    elif options.rem_hot_spare:
        if not (options.type is not None and options.ctrlr_id is not None):
            parser.error('--type and --ctrlr-id are required options')
        if not (options.rem_encl_id is not None and options.rem_slot_num is not None):
            parser.error('--rem-encl-id and --rem-slot-num are required options')
        if options.type == 'MegaRAID':
            if megaraid.working:
                megaraid.rem_hsp_drive(options.ctrlr_id, options.rem_encl_id, options.rem_slot_num)
        elif options.type == 'AACRAID':
            if aacraid.working:
                aacraid.rem_hsp_drive(options.ctrlr_id, options.rem_encl_id, options.rem_slot_num)
